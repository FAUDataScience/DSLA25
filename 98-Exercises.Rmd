```{r setupexercises, echo=TRUE, message=FALSE, warning=FALSE}
...
It's simple to generate a single random variable, but we often want to examine data for several variables that are correlated - perhaps like SAT scores and HS GPA. The MVRnorm function, in the MASS package, does this.
...
#### (Asymmetrical) Venn Diagrams in R
...
## more on bibliometrics: Combining network analysis with simple text analysis
...
library(tidyverse)
library(tidytext)
dataDir <- "data/CompSocSci"
```

### first analysis: what are the top papers in each community (module) of the network?
...
### second analysis: what are the words that characterize "computational social science"?

These use the tidytext approach to text analysis that we saw in chapter 21, which begins by restructuring texts as data frames. Both the size of this vector (the total number of words or tokens) and the first few terms are shown.

Then we order the list of the top 100 terms, after dropping 'stop words' like "a" and "the". (Remember, when we are looking at a single corpus, we will typically want to remove stop words, but when we are comparing corpora, we will typically want to keep them in).
...
### third analysis: How does the language of different communities differ?

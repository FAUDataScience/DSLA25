```{r setup23b, echo=TRUE, message=FALSE, warning=FALSE}
...
smallBiblioDF %>% 
    kable (caption = "A small citation network: 
           Edge list of papers cited > 1 time in five sources in CSS") %>% 
    kable_styling()
...
smallgraph <- smallBiblioDF %>%
    qgraph::qgraph(labels = T, 
           ...
           title = "A small citation network: Common cites among five papers")
```
...
### exploring the communities using Gephi

Another way to see, understand, and share this community structure is to use an interactive network tool called [Gephi](https://gephi.org/). We can prepare the data for Gephi in two ways. The most useful will be to write the nodes and edges to disk in two separate writes:
...
A second approach is to prepare a single network file that is directly readable by a new Web-based tool called [Gephi Lite](https://gephi.org/lite/) (as well as by Gephi). The lite version lacks some of the power (and complexity) of Gephi, but it does demonstrate how one can quickly generate attractive graphs using this popular interactive tool. There are two tutorials and a manual [here](https://docs.gephi.org/lite/).
...
weights <- as.numeric(edgeList[["edgesWeight"]])
  
write.gexf(nodes = nodes, 
           edges = edges,
           edgesWeight = weights,
           output = (file.path(dataDir, "CSS.gexf")))

```

### understanding the communities using text analysis

Another way to understand the communities is to compare the language of different communities, using the combined text fields. That approach is explored in the supplementary code at the end of the exercises chapter.

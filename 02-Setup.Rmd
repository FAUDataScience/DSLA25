```{r setupsetup, include=FALSE, message = FALSE}
...
**Statistics** is of fundamental importance, not just for understanding abstract trends, but for making decisions about everyday life. Consider the case of a senior undergraduate who wants to go to med school. How many schools should she apply to? (see exercise 2.1, below)
...
You may have worked with data in spreadsheets such as Microsoft Excel or Google Sheets (see exercise 2.2). Spreadsheets are handy tools, particularly for smaller datasets. The first spreadsheet program, VisiCalc, was the first "killer app" to usher in the personal computer revolution. But spreadsheets have limitations as well.

**Best practices** have been proposed for using spreadsheets in data science such as, for example, including only data (and not calculations) in spreadsheets, using what we will recognize as a 'tidy' format in which data are in a simple rectangle (that is, avoid combining cells and using multi-line headers), and saving spreadsheets as simple text files, typically in comma-delimited or CSV format [@broman2018]. Typically, this means that each worksheet will be in a separate file, rather than in workbooks with multiple tabs.
...
## AI and the 15-minute rule

An important determinant of your success - not just in this class, but in life - will be the balance you maintain between persistence and help-seeking.

Prior to the age of AI, asking for help with a coding problem implied imposing a burden on another person (near or more typically far, on bulletin boards such as s[tackoverflow.com](https://stackoverflow.com)). We do not and should not treat such burdens lightly, so we would hesitate before help-seeking, and take some time to frame our questions carefully and explicitly.

The 15-minute rule provided a guideline for the balance between persistence and help-seeking: It has been cleverly summarized as "[You must try, and then you must ask](https://www.mattringel.com/2013/09/30/you-must-try-and-then-you-must-ask/)."[^02-setup-1] More explicitly, when you get stuck, you should keep trying for 15 minutes, then reach out to others.

[^02-setup-1]: Contra Yoda was wrong in the claim that "Do or do not. There is no try."

The rule is a constructive one, particularly if it is applied with cognitive flexibility, social sensitivity, and reciprocity. The idea is that when you get stuck, you should make a note of the problem, then move to another part of your project (that's the cognitive flexibility part): This allows your problem to percolate and still make progress. Then, when you ask others for help, ask in a way that shows an awareness of the demands on their time (social sensitivity): Part of this means that you should explain your problem in as detailed a fashion as possible - in technical terms, a "reprex" or reproducible example. Finally, when others ask for help, you should reciprocate or even [pay it forward](https://www.youtube.com/watch?v=TlZDDACt8Nw).

One of the characteristics of the age of AI is that there is no 'friction' that might slow down a call for help - that when you run across a problem, including a coding problem, you can instantly open a window with a chatbot, paste a screenshot of where you are, perhaps add a few words, and then get an answer that might well be what you are looking for.

That's not ideal. Struggle on your own for at least a few minutes. Work at the edge of your competency, push the envelope and grow until your legs reach the ground (see preface).
...
>
> **2.2** Open the Google Sheet at <http://bit.ly/dslaX2_1>. Save a copy and edit it, entering the following in cell B7:
>
> =average (B2:B6)
>
> What is the result?
...
>
> **2.3** What is the rationale for the argument that spreadsheets should be used primarily to store, and not to analyze, data?
>
> **2.4** What is the 15-minute rule, and how should it be adjusted in the age of AI?
>
> ### 

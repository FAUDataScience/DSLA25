...
```{r setup19, echo=TRUE, message=FALSE, warning=FALSE}
...
At the end of the last chapter, we encountered an error. We were trying to predict the dichotomous outcome affairYN - but we forgot to drop the two other variables that were derived from nbaffairs (the original variable) - rootAffair and nbaffairs itself. Note that we did this correctly in the prior analysis of 'rootAffair'. At any rate, here's one way to fix it:
...
trainFair <- trainFair %>%
    select(-nbaffairs, -rootAffair)
```

, and then run a logistic regression model to predict affairYN from the other variables. We will use this model to generate predicted scores, which we will then classify as "Predictunfaithful" or "PredictOK" based on a decision threshold.
...
As scores on predictTrain increase, the predicted likelihood of an affair increases. If you are considering marriage to a potential partner, what score would be too high? How do we distinguish "ok" from "not"? If we want to classify people, we will need to create a decision threshold at which we will change our prediction from 'no' to 'yes'.

## from regression to classification: selection of a threshold

If we want to simply maximize overall prediction, we should create a threshold equal to the actual proportion of people who don't have affairs in our sample:
...
In many decision problems, there is an asymmetry in the cost of different types of errors: if you are foraging for mushrooms, for example, an error of the form

- *you decide its safe but it is poisonous*

is more costly than the converse,

- *you decide its poisonous and it is safe*.
